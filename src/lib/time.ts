...
export let aboutAMonth = 30 * day;
export let aboutAYear = 365 * day;

let pad = (n: number) => n.toString().padStart(2, '0');
export let formatMs = (ms: number, downToThe?: 'ago' | 'ms' | 'min' | 'day') => {
	...
	if (downToThe === 'ago') {
		...
		if (abs < aboutAMonth) return m.timeAgoWeeks({ count: Math.floor(abs / week) });
		if (abs < aboutAYear) return m.timeAgoMonths({ count: Math.floor(abs / aboutAMonth) });
		return m.timeAgoYears({ count: Math.floor(abs / aboutAYear) });
	}
